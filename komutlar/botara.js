...
exports.run = async (client, msg, args) => {
  let prefix =
    (await db.fetch(`${msg.guild.id}.prefix`)) || client.ayarlar.prefix;
  if (!args[0]) {
    return msg.channel.send(
      new Discord.RichEmbed()
        .setDescription("Lütfen bir bot ID'i giriniz!")
        .setColor("RANDOM")
    );
  }
  request(`https://worldbotlist.ml/api/botlar/${args[0]}`, function(
    error,
    response,
    body
  ) {
    ...
    request(`https://worldbotlist.ml/api/tumbotlar`, function(
      errorr,
      responsee,
      bodyy
    ) {
      if (errorr) return msg.channel.send("Hata:", errorr);
        ...
        if (bodyy.includes(args[0]) === false)
          return msg.reply("Bu ID'de bir bot sistemde bulunmamaktadır!");
      }
    });
    ...
    const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setThumbnail(c)
      .setTitle(`KasapBotList - Bot Arama`)
      ...
      .setFooter("http://www.kasap-botlist.cf/ sisteminde bot aramaktadır.");
