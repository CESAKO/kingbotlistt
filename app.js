...
module.exports = client => {
  ...
  app.get("/alim", (req, res) => {
    renderTemplate(res, req, "alim.ejs");
  });
  ...
  app.post("/botekle", checkAuth, (req, res) => {
    ...
    if (
      ...
      return res.redirect("/botyonetim/hata");

    let ID = ayar["botid"];

    if (db.has("botlar")) {
      if (Object.keys(db.fetch("botlar")).includes(ID) === true)
        return res.redirect("/botekle/hata");
    }
    ...
    request(
      ...
      function(error, response, body) {
        if (error) return console.log(error);
          ...
          db.set(
            `botlar.${ID}.isim`,
            sistem.username + "#" + sistem.discriminator
          );

          db.set(
            `botlar.${ID}.avatar`,
            `https://cdn.discordapp.com/avatars/${sistem.id}/${sistem.avatar}.png`
          );
          ...
          request(
            ...
            function(error, response, body) {
              if (error) return console.log(error);
                ...
                db.set(
                  `botlar.${ID}.sahip`,
                  sahip.username + "#" + sahip.discriminator
                );
                ...
                client.channels
                  .get(client.ayarlar.kayıt)
                  .send(
                    `\`${req.user.username}#${req.user.discriminator}\` adlı kullanıcı \`${sistem.id}\` ID'ine sahip \`${sistem.username}#${sistem.discriminator}\` adlı botu ile başvuru yaptı onaylanmayı bekliyor! @☄・Kasap・Bot List Yetkili Kardeş ne güne duruyonuz kontrol edin.`
                  );
                ...
                if (client.users.has(req.user.id) === true) {
                  client.users
                    .get(req.user.id)
                    .send(
                      `\`${sistem.username}#${sistem.discriminator}\` adlı botunuz sitemize eklendi incelendikten sonra eklenicek.`
                    );
                }
              }
            }
            ...
            function(error, response, body) {
              if (error) return console.log(error);
                ...
                client.channels
                  .get(client.ayarlar.kayıt)
                  .send(
                    `\`${req.user.username}#${req.user.discriminator}\` adlı kullanıcı \`${sistem.id}\` ID'ine sahip \`${sistem.username}#${sistem.discriminator}\` adlı botunun başvurusunu/profilini düzenlendi!`
                  );
              }
            }
          );
  ...
  app.post("/bot/:botID/rapor", checkAuth, (req, res) => {
    ...
    if (ayar["mesaj-1"]) {
      db.push(
        `botlar.${req.params.botID}.raporlar`,
        JSON.parse(`{ "rapor":"${ayar["mesaj-1"]}" }`)
      );
      client.channels
        .get("546057097469624320")
        .send(
          `\`${req.user.username}#${
            req.user.discriminator
          }\` adlı kullanıcı \`${db.fetch(
            `botlar.${req.params.botID}.isim`
          )}\` adlı botu raporladı! \n**Sebep:** \`${ayar["mesaj-1"]}\``
        );
    }
    if (ayar["mesaj-2"]) {
  ...
  app.get("/bot/:botID/oyver", checkAuth, (req, res) => {
    ...
    var saat = `${new Date().getHours() +
      3}:${new Date().getMinutes()}:${new Date().getSeconds()}`;
  ...
  app.get(
    "/kullanici/:userID/panel/:botID/sertifikaiste",
    checkAuth,
    (req, res) => {
      var id = req.params.botID;
  ...
  app.post(
    ...
    (req, res, body) => {
      ...
      let ID = req.params.botID;
      let s = req.user.id;
      db.set(`botlar.${ID}.sertifikaistek`, "Sertifika İsteği");
      request(
        {
          url: `https://discordapp.com/api/v7/users/${ID}`,
          headers: {
        ...
        function(error, response, body) {
          if (error) return console.log(error);
            ...
            client.channels
              .get(client.ayarlar.kayıt)
              .send(
                `\`${req.user.username}#${req.user.discriminator}\` adlı kullanıcı \`${sistem.id}\` ID'ine sahip \`${sistem.username}#${sistem.discriminator}\` adlı botuna sertifika istedi`
              );
            const embed = new Discord.RichEmbed()
              ...
              .addField(
                "Botunuzun aynı işlevi yapan diğer botlardan farkı varmı? Varsa neler",
                req.body["secim3"]
              );
  ...
  app.get("/yetkili", checkAuth, (req, res) => {
    if (!client.yetkililer.includes(req.user.id))
      return res.redirect("/yetkili/hata");
  ...
  app.get("/botyonetici/onayla/:botID", checkAuth, (req, res) => {
    ...
    client.channels
      ...
      .send(
        `\`${req.user.username}#${
          req.user.discriminator
        }\` adlı yetkili tarafından \`${db.fetch(
          `botlar.${id}.sahip`
        )}\` adlı kullanıcının \`${db.fetch(
          `botlar.${id}.id`
        )}\` ID'ine sahip \`${db.fetch(
          `botlar.${id}.isim`
        )}\` adlı botu onaylandı!`
      );
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        .get(db.fetch(`botlar.${id}.sahipid`))
        .send(
          `\`${db.fetch(
            `botlar.${id}.isim`
          )}\` adlı botunuz onaylandı! \n https://kasap-botlist.glitch.me/bot/${db.fetch(
            `botlar.${id}.id`
          )}`
        );
    }
    ...
    client.channels
      ...
      .send(
        `\`${req.user.username}#${
          ...
          `botlar.${id}.isim`
        )}\` adlı botu bekleme moduna alındı!`
      );
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        ...
        .send(
          `\`${db.fetch(
            `botlar.${id}.isim`
          )}\` adlı botunuz beklemeye/incelenmeye alındı!`
        );
    }
  });
  ...
  app.post("/botyonetici/reddet/:botID", checkAuth, (req, res) => {
    ...
    client.channels
      ...
      .send(
        `\`${req.user.username}#${
          ...
          `botlar.${id}.id`
        )}\` ID'ine sahip \`${db.fetch(`botlar.${id}.isim`)}\` adlı botu \`${
          req.body["red-sebep"]
        }\` sebebi ile reddedildi!`
      );
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        .get(db.fetch(`botlar.${id}.sahipid`))
        .send(
          `\`${db.fetch(`botlar.${id}.isim`)}\` adlı botunuz \`${
            req.body["red-sebep"]
          }\` sebebi ile reddedildi!`
        );
    }
  ...
  app.get("/botyonetici/sertifikaver/:botID", checkAuth, (req, res) => {
    ...
    db.delete(`botlar.${id}.sertifikaistek`);
    res.redirect("/yetkili");
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        ...
        .send(
          `\`${db.fetch(
            `botlar.${id}.isim`
          )}\` adlı botunuza sertifika verildi!`
        );
    }
  });

  app.get("/botyonetici/sertifikaverme/:botID", checkAuth, (req, res) => {
    if (!client.yetkililer.includes(req.user.id))
      return res.redirect("/yetkili/hata");
    renderTemplate(res, req, "sertifikaverme.ejs");
  });

  app.post("/botyonetici/sertifikaverme/:botID", checkAuth, (req, res) => {
    if (!client.yetkililer.includes(req.user.id))
      return res.redirect("/yetkili/hata");
    let id = req.params.botID;
    db.delete(`botlar.${id}.sertifikaistek`);
    ...
    client.channels
      ...
      .send(
        `\`${req.user.username}#${
          ...
          req.body["sertifika-red-sebep"]
        }\` sebebi ile sertifika verilmedi.`
      );
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        .get(db.fetch(`botlar.${id}.sahipid`))
        .send(
          `\`${db.fetch(`botlar.${id}.isim`)}\` adlı botunuza \`${
            req.body["sertifika-red-sebep"]
          }\` sebebi ile sertifika verilmedi!`
        );
    }
    ...
    client.channels
      ...
      .send(
        `\`${req.user.username}#${
          ...
          `botlar.${id}.id`
        )}\` ID'ine sahip botunun sertifikası \`${
          req.body["sertifika-sil-sebep"]
        }\` sebebi ile silindi.`
      );
    ...
    if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
      client.users
        ...
        .send(
          `\`${db.fetch(
            `botlar.${id}.isim`
          )}\` adlı botunda bulunan sertifika \`${
            req.body["sertifika-sil-sebep"]
          }\` sebebi ile silindi!`
        );
    }
  ...
  app.get("/api/botlar/:botID", (req, res) => {
    ...
    res.json({
      ...
      durum: db.has(`botlar.${id}.durum`)
        ? db.fetch(`botlar.${id}.durum`)
        : "Beklemede",
