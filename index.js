...
client.yetkililer = ["479704347509522453", "605990400666107905"]; //tüm yetkililerin ıdleri gelcek array
client.webyetkililer = ["479704347509522453", "605990400666107905"]; //web yetkililerin ıdleri gelcek array
client.sunucuyetkililer = ["771265578819911700", "605990400666107905"]; //sunucu yetkililerin ıdleri gelcek array
...
client.on("ready", async () => {
  ...
  client.user.setActivity(`${client.ayarlar.prefix}yardım`, {
    type: "PLAYING"
  });
  ...
  if (cmd) {
    if (cmd.conf.permLevel === "ozel") {
      ...
      if (client.yetkililer.includes(message.author.id) === false) {
        const embed = new Discord.RichEmbed()
          .setDescription(
            `Kardeşim sen WebSite yetkilisi değilsin saçma saçma işlerle uğraşma!`
          )
    ...
    if (cmd.conf.permLevel === 1) {
      if (!message.member.hasPermission("MANAGE_MESSAGES")) {
        const embed = new Discord.RichEmbed()
          .setDescription(
            `Sen önce mesajları yönetmeyi öğren sonra bu komutu kullanırsın.`
          )
          .setColor(client.ayarlar.renk)
          .setTimestamp();
        message.channel.send("Yetersiz yetki.");
...
client.login("Nzg5NDU3Mzk5NDMyODcxOTM2.X9yVfw.U6c1oC2tfLVyNqE6Slj3icoA5mA"); //tokeni yaz işte

process.env = {};
process.env.TOKEN =
  "Nzg5NDU3Mzk5NDMyODcxOTM2.X9yVfw.U6c1oC2tfLVyNqE6Slj3icoA5mA";
